...
void App::render() {
    ...
    if(game_.isPlayerTurn() && game_.getPlayerColor() == Game::StoneColor::Red
        || ! game_.isPlayerTurn() && game_.getPlayerColor() == Game::StoneColor::Black)
...
void App::update() {
    ...
    // 更新box
    if(game_.getSelectedStone() != nullptr) {
        // 选中某个棋子
        Stone *selected = game_.getSelectedStone();
    ...
    for(int i=0; i<StoneMap::stone_cnt_; i++) {
        if(! stones[i].alive_) {
            ...
            sf::Vector2f pos(stones[i].location_.x*(settings.stone_.radius_ << 1) + settings.padding_.left_ + settings.stone_.radius_, 
                stones[i].location_.y*(settings.stone_.radius_ << 1) + settings.padding_.top_ + settings.stone_.radius_);
...
void App::mouseClickedHandler(sf::Event &event) {
        ...
        || y < settings.padding_.top_ || y > settings.padding_.top_ + r * 20)
    {
        ...
        else if(undo_button_.getGlobalBounds().contains(x, y) && !game_.isGameOver() ) {
            game_.regret();
        }
    }
...
void App::mouseMovedHandler(sf::Event &event) {
    Settings &settings = Settings::getInstance();
    ...
    const int window_width = settings.window_size_.width;
    const int window_height = settings.window_size_.height;
...
App::App() {
    ...
    { // 窗口及图标设置
        window_.create(sf::VideoMode(settings.window_size_.width, settings.window_size_.height), 
            sf::String::fromUtf8(settings.title_.begin(), settings.title_.end()), 
    ...
    { // 棋盘设置
        ...
        float scalex = settings.window_size_.width / (float)board_bg_texture.getSize().x;
        float scaley = settings.window_size_.height / (float)board_bg_texture.getSize().y;
        ...
        river_.setPosition((settings.window_size_.width - settings.panel_width_) / 2.0, 
            settings.window_size_.height / 2.0);
    ...
    { // 棋子设置
        ...
        for(int i=0; i<StoneMap::stone_cnt_; i++) {
            ...
            // 根据棋盘坐标计算实际坐标
            sf::Vector2f pos(stones[i].location_.x * 2 * settings.stone_.radius_ + settings.stone_.radius_ + settings.padding_.left_, 
                stones[i].location_.y * 2 * settings.stone_.radius_ + settings.stone_.radius_ + settings.padding_.top_);
            stone_background_[i].setPosition(pos);
            stone_text_[i].setPosition(pos);

            if(i < 16 && game_.getPlayerColor() == Game::StoneColor::Red || i >= 16 && Game::StoneColor::Black == game_.getPlayerColor()) {
                stone_text_[i].setColor(black);
            }
            else {
                stone_text_[i].setColor(red);
            }
        }
    }
    ...
    { // 当前玩家标签
        ...
        const int pos_x = settings.window_size_.width - settings.panel_width_ + turn_label_left;
    ...
    { // 控制按钮
        ...
        sf::FloatRect bound = new_game_button_.getLocalBounds();
        new_game_button_.setOrigin(bound.width / 2.0, bound.height);
        new_game_button_.setPosition(settings.window_size_.width - settings.panel_width_ / 2, 
            settings.window_size_.height / 2);

        bound = undo_button_.getLocalBounds();
        undo_button_.setOrigin(bound.width / 2.0, 0);
        undo_button_.setPosition(settings.window_size_.width - settings.panel_width_ / 2, settings.window_size_.height / 2);
...

    {
        ...
        looog_.setPosition((settings.window_size_.width - settings.panel_width_) / 2.0, 
            settings.window_size_.height / 2.0);
        ...
        win_.setPosition((settings.window_size_.width - settings.panel_width_) / 2.0, 
            settings.window_size_.height / 2.0);
        ...
        loose_.setPosition((settings.window_size_.width - settings.panel_width_) / 2.0, 
            settings.window_size_.height / 2.0);
